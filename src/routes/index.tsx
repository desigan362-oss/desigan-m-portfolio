...
import uxHero from "@/assets/uxlearn-hero.jpg";
...
function Index() {
  ...
  return <>
    <section className="relative min-h-[calc(100svh-4rem)] overflow-hidden bg-panel px-6 pb-12 pt-12 text-surface md:pt-20">
      <div className="pointer-events-none absolute inset-y-0 right-[12%] w-px bg-surface/10" />
      <div className="mx-auto grid min-h-[calc(100svh-8rem)] max-w-7xl items-center gap-12 lg:grid-cols-[1.05fr_.95fr]">
        <div className="relative z-10">
          ...
          <p className="mt-7 max-w-2xl text-xl font-medium leading-snug text-surface/85 md:text-3xl">I shape ideas into useful experiences, distinctive brands and confident designers.</p>
          <p className="mt-5 max-w-xl text-sm leading-relaxed text-surface/55 md:text-base">UI/UX Designer · Graphic Designer · Visual Designer · Trainer</p>
          <div className="mt-9 [&_a]:!border-surface/20 [&_a]:!bg-surface/5 [&_a]:!text-surface"><ContactActions /></div>
        </div>
        <div className="relative mx-auto h-[440px] w-full max-w-xl overflow-hidden md:h-[620px]">
          <div className="absolute left-1/2 top-6 h-[74%] w-[74%] -translate-x-1/2 rounded-full border-[22px] border-accent/80 md:border-[34px]" />
          <img src={portrait.url} alt="Desigan M." width={800} height={1000} className="absolute bottom-0 left-1/2 z-10 h-[92%] w-auto max-w-none -translate-x-1/2 object-contain object-bottom" />
          <div className="absolute bottom-4 left-0 z-20 w-[38%] [--phone-rotate:-7deg] animate-rise rounded-[24px] border-4 border-panel bg-panel p-1 shadow-2xl [animation-delay:200ms]"><img src={bakeryHero} alt="Bakery mobile app design" className="aspect-[9/18] w-full rounded-[18px] object-cover" /></div>
          <div className="absolute bottom-1 right-0 z-20 w-[38%] [--phone-rotate:7deg] animate-rise rounded-[24px] border-4 border-panel bg-panel p-1 shadow-2xl [animation-delay:420ms]"><img src={uxHero} alt="UX learning mobile interface" className="aspect-[9/18] w-full rounded-[18px] object-cover" /></div>
          <div className="absolute right-0 top-0 z-30 max-w-[190px] rounded-md border border-surface/15 bg-panel-soft/90 p-4 text-xs leading-relaxed text-surface/70 backdrop-blur"><span className="mb-2 block font-bold text-coral">Design note 01</span>Good design is not decoration. It is a clear decision made visible.</div>
        </div>
      </div>
      <a href="#proof" className="absolute bottom-5 left-1/2 z-30 flex -translate-x-1/2 items-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em] text-surface/45">Explore <ArrowDown className="size-4 animate-bounce" /></a>
    </section>
    ...
    <section className="bg-panel px-6 py-24 text-surface md:py-32">
      <div className="mx-auto max-w-7xl">
        <div className="mb-14 flex flex-wrap items-end justify-between gap-6"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-accent">Portfolio highlights</p><h2 className="mt-4 font-display text-4xl font-bold md:text-6xl">Featured engagements</h2></div><Link to="/work" className="flex items-center gap-2 border-b border-surface/30 pb-1 text-sm font-bold">Explore all work <ArrowUpRight className="size-4" /></Link></div>
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 [&_a]:!text-ink">{featured.slice(0, 6).map((project, index) => <ProjectCard key={project.id} project={project} index={index} />)}</div>
      </div>
    </section>

    <section className="px-6 py-24 md:py-32">
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-14 lg:grid-cols-[.7fr_1.3fr]"><div><GraduationCap className="size-8 text-accent" /><p className="mt-6 text-xs font-bold uppercase tracking-[0.2em] text-accent">Learning journey</p><h2 className="mt-4 font-display text-4xl font-bold md:text-6xl">Certified through practice.</h2><p className="mt-5 max-w-sm leading-relaxed text-ink/65">Internships across product and visual design, followed by formal trainer certifications.</p></div><div className="border-t border-ink/15">{certificates.map(([period,title,place], index) => <div key={title} className="grid gap-3 border-b border-ink/15 py-6 sm:grid-cols-[150px_1fr_auto] sm:items-center"><span className="text-xs font-bold text-accent">{period}</span><strong>{title}</strong><span className="text-sm text-ink/50">{place}</span></div>)}</div></div>
      </div>
    </section>
  </>;
}
