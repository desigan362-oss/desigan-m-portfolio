      ...
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-12 lg:grid-cols-[.75fr_1.25fr]">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {[[Palette,"Graphic design","Brand identity, packaging, campaigns and visual communication."],[PenTool,"UI/UX design","User-centred flows, interfaces, prototypes and scalable systems."],[Users,"Training","Practical mentoring that has helped 35+ students build design confidence."]].map(([Icon,title,copy], index) => { const I = Icon as typeof Palette; return <div key={String(title)} className={`${index === 2 ? "bg-pastel-mint" : index === 1 ? "bg-pastel-blue" : "bg-pastel-peach"} rounded-md border border-ink/15 p-6 text-ink shadow-sm transition-transform hover:-translate-y-1`}><I className="size-6" /><h3 className="mt-10 text-lg font-bold">{String(title)}</h3><p className="mt-3 text-sm leading-relaxed text-ink/80">{String(copy)}</p></div>})}
          </div>
        </div>
        <div className="mt-14 grid grid-cols-2 gap-px overflow-hidden rounded-md border border-ink/15 bg-ink/10 md:grid-cols-4">{[["5+","Clients"],["4","UI/UX projects"],["35+","Students trained"],["100+","Visual designs"]].map(([value,label], index) => <div key={label} className={`${["bg-pastel-lilac","bg-pastel-blue","bg-pastel-mint","bg-pastel-peach"][index]} p-6 text-ink md:p-8`}><strong className="font-display text-4xl font-bold text-accent md:text-5xl">{value}</strong><span className="mt-2 block text-xs font-bold uppercase text-ink/70">{label}</span></div>)}</div>
      </div>
    </section>
